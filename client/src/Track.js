...
function Track() {
    ...
    const loadBlockchaindata = async () => {
        ...
        if (networkData) {
            ...
            const ProdStage = [];
            for (i = 0; i < ProdCtr; i++) {
                Prod[i + 1] = await supplychain.methods.productStock(i + 1).call();
                ProdStage[i + 1] = await supplychain.methods.showStage(i + 1).call();
            }
            setProd(Prod);
            ...
            <div className="container mt-4">
                <div className="card mb-4">
                    ...
                    <div className="card-body">
                        <p><b>Product ID:</b> {Prod[ID].id}</p>
                        <p><b>Name:</b> {Prod[ID].name}</p>
                        <p><b>Description:</b> {Prod[ID].description}</p>
                        <p><b>Current stage:</b> {ProdStage[ID] && ProdStage[ID].replace(/Prodicine|Prod|Prod|Prod/g, 'Product')}</p>
                    </div>
                </div>
        ...
        return (
            <div className="container mt-4">
                <article className="col-4">
                    ...
                    <br />
                    <span><b>Current stage: </b>{ProdStage[ID] && ProdStage[ID].replace(/Product|Prod|Prod|Prod/g, 'Product')}</span>
                    <hr />
                    <br />
                    ...
                    <tbody>
                        {Object.keys(Prod).map((key) => (
                            <tr key={key}>
                                ...
                                <td>{ProdStage[key] && ProdStage[key].replace(/Product|Prod|Prod|Prod/g, 'Product')}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
